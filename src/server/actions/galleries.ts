...
export interface GetGalleriesOptions {
  ...
  limit?: number;
  offset?: number;
  orderBy?: 'createdAt' | 'publishedAt' | 'viewCount' | 'likeCount' | 'captureDate';
  order?: 'asc' | 'desc';
}
...
export async function createGallery(input: CreateGalleryInput) {
  try {
    ...
    if (input.photos && input.photos.length > 0) {
      ...
      const { error: photoError } = await supabase.from('PhotoImage').insert(photos);
...
export async function deleteGallery(id: string) {
  try {
    ...
    await supabase.from('Like').delete().eq('targetType', 'GALLERY').eq('targetId', id);
 ...
 * 添加照片到相册
 */
export async function addPhotosToGallery(galleryId: string, photos: CreatePhotoInput[]) {
  try {
...
export async function deletePhoto(photoId: string) {
  try {
    ...
    // 删除照片
    const { error } = await supabase.from('PhotoImage').delete().eq('id', photoId);
    if (error) throw error;
 ...
 * 批量更新照片顺序
 */
export async function updatePhotosOrder(updates: { id: string; order: number }[]) {
  try {
    for (const update of updates) {
      await supabase.from('PhotoImage').update({ sortOrder: update.order }).eq('id', update.id);
    }
